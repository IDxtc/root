...
EOF
		...
		echoContent yellow " ---> Universal vmess(VMess+gRPC+TLS)Link"
		echoContent green "    vmess://${qrCodeBase64Default}\n"
...
EOF
		...
		qrCodeBase64Default=$(echo -n "{:\"v\":2,\"ps\":\"${email}\",\add\":\"${add}\",\"port"\:${port},\"id\":\"${id}\",\"aid\":0,\"net\":\"grpc\",\"serviceName\":\"${path}\",\"type\":\"grpc\",\"allowInsecure\":0,\"sni\":\"${host}\"}" | base64 -w 0)
		qrCodeBase64Default="${qrCodeBase64Default// /}"
...
addUser() {
	...
	while [[ ${userNum} -gt 0 ]]; do
		...
		if echo ${currentInstallProtocolType} | grep -q 6; then
			...
			vmessWsResult=$(jq -r ".inbounds[0].settings.clients += [${vmessUsers}]" ${configPath}07_VMess_gRPC_inbounds.json)
...
removeUser() {
	...
	if [[ -n "${delUserIndex}" ]]; then
		...
		if echo ${currentInstallProtocolType} | grep -q 6; then
			...
			echo "${vmessWSResult}" | jq . >${configPath}07_VMess_gRPC_inbounds.json
		fi
