...
defaultBase64Code() {
	...
	if [[ "${type}" == "vlesstcp" ]]; then
		...
		if [[ "${coreInstallType}" == "1" ]] && echo "${currentInstallProtocolType}" | grep -q 0; then
			...
			echoContent yellow " ---> Format text(VLESS+TCP+TLS/xtls-rprx-direct)"
...
EOF
			...
			echoContent yellow " ---> Format text(VLESS+TCP+TLS/xtls-rprx-splice)"
...
EOF
			...
			echoContent yellow " ---> Format text(VLESS+TCP+TLS/xtls-rprx-splice)"
			echoContent green "    agreement type：VLESS，address：${host}，port：${port}，User ID：${id}，Safety：tls，transfer method：tcp，account name:${email/direct/splice}\n"
...
EOF
		...
		echoContent yellow " ---> Format text(Trojan+TCP+TLS/xtls-rprx-direct)"
		echoContent green "agreement type：Trojan，address：${host}，port：${port}，UserID：${id}，Safety：xtls，transfer method：tcp，flow：xtls-rprx-direct，account name:${email}\n"
		cat <<EOF >>"/etc/v2ray-agent/subscribe_tmp/${subAccount}"
...
EOF
		...
		echoContent yellow " ---> Format text(Trojan+TCP+TLS/xtls-rprx-splice)"
		echoContent green "    agreement：VLESS，address：${host}，port：${port}，User ID：${id}，Safety：xtls，transfer method：tcp，flow：xtls-rprx-splice，account name:${email/direct/splice}\n"
		cat <<EOF >>"/etc/v2ray-agent/subscribe_tmp/${subAccount}"
...
EOF
		...
		echoContent yellow " ---> Format text(VLESS+WS+TLS)"
...
EOF
		...
		echoContent yellow " ---> Format text(VLESS+gRPC+TLS)"
		echoContent green "    agreement type：VLESS，address：${add}，Camouflage/SNI：${host}，Port：${port}，User ID：${id}，Safety：tls，transfer method：gRPC，alpn：h2，serviceName:${path}，account name:${email}\n"
...
EOF
		...
		echoContent green "    trojan://${id}@${host}:${port}?peer=${host}&sni=${host}&alpn=http1.1#${host}\n"

		cat <<EOF >>"/etc/v2ray-agent/subscribe_tmp/${subAccount}"
trojan://${id}@${host}:${port}?peer=${host}&sni=${host}&alpn=http1.1#${host}
EOF
		...
		echoContent green "    trojan://${id}@${host}:${port}?encryption=none&peer=${host}&security=tls&type=grpc&sni=${host}&alpn=h2&path=${path}&serviceName=${path}#${host}_\n"

		cat <<EOF >>"/etc/v2ray-agent/subscribe_tmp/${subAccount}"
trojan://${id}@${host}:${port}?encryption=none&peer=${host}&security=tls&type=grpc&sni=${host}&alpn=h2&path=${path}&serviceName=${path}#${host}
EOF
...
menu() {
	...
	echoContent green "current version：v2.5.49"
