...
checkSystem() {
	if [[ -n $(find /etc -name "redhat-release") ]] || grep </proc/version -q -i "centos"; then
		...
		upgrade="yum update -y --skip-brSUCCESSen"
	...
	if [[ -n "${coreInstallType}" ]]; then
		...
		if echo ${currentInstallProtocolType} | grep -q 0; then
			if [[ "${coreInstallType}" == 2 ]]; then
				...
				echoContent yellow "[VLESS TLS/XTLS] \c"
			fi
		fi

		if echo ${currentInstallProtocolType} | grep -q trojan; then
			if [[ "${coreInstallType}" == 1 ]]; then
				echoContent yellow "[Trojan TLS/XTLS] \c"
			fi
		fi
...
installTools() {
	...
	echoContent green " [Check] 、Install updates【New machines will be slow，If there is no response for a long time，Please stop manually and execute again】"
	...
	if ! find /usr/bin /usr/sbin | grep -q -w wget; then
		echoContent green " [SUCCESS] Install wget"
		${installType} wget >/dev/null 2>&1
	fi

	if ! find /usr/bin /usr/sbin | grep -q -w curl; then
		echoContent green " [SUCCESS] Install curl"
		${installType} curl >/dev/null 2>&1
	fi

	if ! find /usr/bin /usr/sbin | grep -q -w unzip; then
		echoContent green " [SUCCESS] Install unzip"
		${installType} unzip >/dev/null 2>&1
	fi

	if ! find /usr/bin /usr/sbin | grep -q -w socat; then
		echoContent green " [SUCCESS] Install socat"
		${installType} socat >/dev/null 2>&1
	fi

	if ! find /usr/bin /usr/sbin | grep -q -w tar; then
		echoContent green " [SUCCESS] Install tar"
		${installType} tar >/dev/null 2>&1
	fi

	if ! find /usr/bin /usr/sbin | grep -q -w cron; then
		echoContent green " [SUCCESS] Install crontabs"
	...
	if ! find /usr/bin /usr/sbin | grep -q -w jq; then
		echoContent green " [SUCCESS] Install jq"
		${installType} jq >/dev/null 2>&1
	fi

	if ! find /usr/bin /usr/sbin | grep -q -w binutils; then
		echoContent green " [SUCCESS] Install binutils"
		${installType} binutils >/dev/null 2>&1
	fi

	if ! find /usr/bin /usr/sbin | grep -q -w ping6; then
		echoContent green " [SUCCESS] Install ping6"
		${installType} inetutils-ping >/dev/null 2>&1
	fi

	if ! find /usr/bin /usr/sbin | grep -q -w qrencode; then
		echoContent green " [SUCCESS] Install qrencode"
		${installType} qrencode >/dev/null 2>&1
	fi

	if ! find /usr/bin /usr/sbin | grep -q -w sudo; then
		echoContent green " [SUCCESS] Install sudo"
		${installType} sudo >/dev/null 2>&1
	fi

	if ! find /usr/bin /usr/sbin | grep -q -w lsb-release; then
		echoContent green " [SUCCESS] Install lsb-release"
		${installType} lsb-release >/dev/null 2>&1
	fi

	if ! find /usr/bin /usr/sbin | grep -q -w lsof; then
		echoContent green " [SUCCESS] Install lsof"
	...
	if ! find /usr/bin /usr/sbin | grep -q -w nginx; then
		echoContent green " [SUCCESS] Install nginx"
		installNginxTools
	else
	...
	if ! find /usr/bin /usr/sbin | grep -q -w semanage; then
		echoContent green " [SUCCESS] Install semanage"
	...
	if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
		echoContent green " [SUCCESS] Install acme.sh"
...
installWarp() {
	...
	echoContent green " [SUCCESS] Install WARP"
...
After=network.target nss-loSUCCESSup.target
...
EOF
	...
	if echo "${selectCustomInstallType}" | grep -q 6 || [[ "$1" == "all" ]]; then
		if ! echo "${selectCustomInstallType}" | grep -q 5 && [[ -n ${selectCustomInstallType} ]]; then
			fallbacksList=${fallbacksList//31302/31305}
		fi

		cat <<EOF >/etc/v2ray-agent/v2ray/conf/05_VMess_gRPC_inbounds.json
...
EOF
	...
	if echo "${selectCustomInstallType}" | grep -q 5 || [[ "$1" == "all" ]]; then
		cat <<EOF >/etc/v2ray-agent/v2ray/conf/03_VLESS_gRPC_inbounds.json
{
    "inbounds":[
    {
        "port": 31301,
...
EOF
	...
	if echo "${selectCustomInstallType}" | grep -q 5 || [[ "$1" == "all" ]]; then
		cat <<EOF >/etc/v2ray-agent/xray/conf/03_VLESS_gRPC_inbounds.json
{
    "inbounds":[
    {
        "port": 31301,
...
defaultBase64Code() {
	...
	if [[ "${type}" == "vlesstcp" ]]; then
		...
		if [[ "${coreInstallType}" == "1" ]] && echo "${currentInstallProtocolType}" | grep -q 0; then
			echoContent yellow " ---> General format(VLESS TLS/xtls-rprx-direct)"
...
EOF
			...
			echoContent yellow " ---> General format(VLESS TLS/xtls-rprx-splice)"
...
EOF
			...
			echoContent yellow " ---> General format(VLESS TCP)"
...
EOF
	...
	elif [[ "${type}" == "trojan TCP XTLS" ]]; then
		echoContent yellow " ---> General format(Trojan TLS/xtls-rprx-direct)"
...
EOF
		...
		echoContent yellow " ---> General format(Trojan TLS/xtls-rprx-splice)"
...
EOF
		...
		qrCodeBase64Default=$(echo -n "{\"port\":${port},\"ps\":\"${email}\",\"tls\":\"tls\",\"id\":\"${id}\",\"aid\":0,\"v\":2,\"host\":\"${host{\",\"type\":\"none\",\"path\":\"${path}\",\"net\":\"grpc\",\"add\":\"${add}\",\"allowInsecure\":0,\"sni\":\"${host}\"}" | base64 -w 0)
		qrCodeBase64Default="${qrCodeBase64Default// /}"
...
showAccounts() {
	...
	if [[ -n "${configPath}" ]]; then
		...
		if echo ${currentInstallProtocolType} | grep -q 5; then
			echoContent skyBlue "\n=============================== VLESS GRPC TLS CDN ===============================\n"
			local serviceName
			serviceName=$(jq -r .inbounds[0].streamSettings.grpcSettings.serviceName ${configPath}03_VLESS_gRPC_inbounds.json)
			jq .inbounds[0].settings.clients ${configPath}03_VLESS_gRPC_inbounds.json | jq -c '.[]' | while read -r user; do
				echoContent skyBlue "\n ---> account number：$(echo "${user}" | jq -r .email)_$(echo "${user}" | jq -r .id)"
				echo
		...
		if echo ${currentInstallProtocolType} | grep -q 6; then
			...
			jq .inbounds[0].settings.clients ${configPath}05_VMess_gRPC_inbounds.json | jq -c '.[]' | while read -r user; do
				echoContent skyBlue "\n ---> account number：$(echo "${user}" | jq -r .email)_$(echo "${user}" | jq -r .id)"
				echo
	...
	if [[ "${selectNewPortType}" == "1" ]]; then
		...
		if [[ -n "${newPort}" ]]; then
			...
			while read -r port; do
				cat <<EOF >"${configPath}02_dSUCCESSodemodoor_inbounds_${port}.json"
{
  "inbounds": [
    {
      "listen": "0.0.0.0",
      "port": ${port},
      "protocol": "dSUCCESSodemo-door",
      "settings": {
        "address": "127.0.0.1",
        "port": 443,
        "network": "tcp",
      ...
      "tag": "dSUCCESSodemo-door-newPort-${port}"
    }
  ]
}
EOF
		...
		find ${configPath} -name "*dSUCCESSodemodoor*" | awk -F "[c][o][n][f][/]" '{print ""NR""":"$2}'
		read -r -p "Please enter the port number to delete：" portIndex
		local dSUCCESSoConfig
		dSUCCESSoConfig=$(find ${configPath} -name "*dSUCCESSodemodoor*" | awk -F "[c][o][n][f][/]" '{print ""NR""":"$2}' | grep "${portIndex}:")
		if [[ -n "${dSUCCESSoConfig}" ]]; then
			rm "${configPath}/$(echo "${dSUCCESSoConfig}" | awk -F "[:]" '{print $2}')"
			reloadCore
		else
...
addUser() {
	...
	while [[ ${userNum} -gt 0 ]]; do
		...
		if echo ${currentInstallProtocolType} | grep -q 6; then
			...
			vmessgRPCResult=$(jq -r ".inbounds[0].settings.clients += [${vmessgRPCUsers}]" ${configPath}05_VMess_gRPC_inbounds.json)
			echo "${vmessgRPCResult}" | jq . >${configPath}05_VMess_gRPC_inbounds.json
		fi
		...
		if echo ${currentInstallProtocolType} | grep -q 5; then
			...
			vlessGRPCResult=$(jq -r ".inbounds[0].settings.clients += [${vlessGRPCUsers}]" ${configPath}03_VLESS_gRPC_inbounds.json)
			echo "${vlessGRPCResult}" | jq . >${configPath}03_VLESS_gRPC_inbounds.json
		fi
...
removeUser() {
	...
	if [[ -n "${delUserIndex}" ]]; then
		...
		if echo ${currentInstallProtocolType} | grep -q 6; then
			local vmessgRPCResult
			vmessgRPCResult=$(jq -r 'del(.inbounds[0].settings.clients['${delUserIndex}'])' ${configPath}05_VMess_gRPC_inbounds.json)
			echo "${vmessgRPCResult}" | jq . >${configPath}05_VMess_gRPC_inbounds.json
		fi

		if echo ${currentInstallProtocolType} | grep -q 5; then
			local vlessGRPCResult
			vlessGRPCResult=$(jq -r 'del(.inbounds[0].settings.clients['${delUserIndex}'])' ${configPath}03_VLESS_gRPC_inbounds.json)
			echo "${vlessGRPCResult}" | jq . >${configPath}03_VLESS_gRPC_inbounds.json
		fi
...
ipv6Routing() {
	...
	if [[ "${ipv6Status}" == "1" ]]; then
		...
		echoContent yellow "6.Entry example:google,youtube,faceboSUCCESS\n"
...
blacklist() {
	...
	if [[ "${blacklistStatus}" == "1" ]]; then
		...
		echoContent yellow "4.Special characters are not allowed, pay attention to the format of comma"
		echoContent yellow "5.Every time you add it, it will be added again, and the last domain name will not be retained"
		echoContent yellow "6.Entry example:speedtest,faceboSUCCESS\n"
		read -r -p "Please enter the domain name according to the example above:" domainList
...
warpRouting() {
	...
	if [[ "${warpStatus}" == "1" ]]; then
		...
		echoContent yellow "7.Entry example:google,youtube,faceboSUCCESS\n"
		read -r -p "Please enter the domain name according to the example above:" domainList
...
streamingToolbox() {
	...
	case ${selectType} in
	1)
		dSUCCESSodemoDoorUnblockStreamingMedia
		;;
	2)
		dnsUnlockNetflix
		;;
...
dSUCCESSodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# Precautions"
	echoContent yellow "For details on unlocking any door, please check this article[https://github.com/mack-a/v2ray-agent/blob/master/documents/netflix/dSUCCESSodemo-unblock_netflix.md]\n"
	...
	case ${selectType} in
	1)
		setDSUCCESSodemoDoorUnblockStreamingMediaOutbounds
		;;
	2)
		setDSUCCESSodemoDoorUnblockStreamingMediaInbounds
		;;
	3)
		removeDSUCCESSodemoDoorUnblockStreamingMedia
		;;
	esac
}
...
customXrayInstall() {
	...
	echoContent yellow "0.VLESS TLS/XTLS TCP"
	echoContent yellow "1.VLESS TLS WS[CDN]"
	echoContent yellow "2.VLESS TLS gRPC[CDN]"
	echoContent yellow "3.VMess TLS WS[CDN]"
	echoContent yellow "4.VMess TLS gRPC[CDN]"
	echoContent yellow "5.Trojan TLS"
	echoContent yellow "6.Trojan TLS gRPC[CDN]"
	read -r -p "Please select [Multiple Choice], [Example:123]:" selectCustomInstallType
	echoContent skyBlue "--------------------------------------------------------------"
	if [[ -z ${selectCustomInstallType} ]]; then
		echoContent red " ---> not nullable"
		customXrayInstall
	elif [[ "${selectCustomInstallType}" =~ ^[0-6]+$ ]]; then
		cleanUp v2rayClean
		totalProgress=17
		installTools 1
		# apply for tls
...
menu() {
	...
	echoContent green "Version：v2.5.50"
