...
EOF
		...
		qrCodeBase64Default=$(echo -n "{\"port\":${port},\"ps\":\"${email}\",\"tls\":\"tls\",\"id\":\"${id}\",\"aid\":0,\"v\":2,\"host\":\"",\"type\":\"none\",\"path\":\"${path}\",\"net\":\"grpc\",\"add\":\"${add}\",\"sni\":\"${host}\"}" | base64 -w 0)
		qrCodeBase64Default="${qrCodeBase64Default// /}"
...
EOF
		...
		echoContent green "    vless://${id}@${add}:${port}?mode=multi&&security=tls&encryption=none&type=grpc&serviceName=${path}&sni=${host}#${email}\n"

		cat <<EOF >>"/etc/v2ray-agent/subscribe_tmp/${subAccount}"
vless://${id}@${add}:${port}?mode=multi&&security=tls&encryption=none&type=grpc&serviceName=${path}&sni=${host}#${email}
EOF
