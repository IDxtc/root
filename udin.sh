...
# Checking 80、443 Port usage
checkPortUsedStatus() {
	if lsof -i tcp:80 | grep -q LISTEN; then
		echoContent red "\n ---> 80 The port is occupied, please close it manually and then install\n"
		lsof -i tcp:80 | grep LISTEN
		exit 0
	fi

	if lsof -i tcp:443 | grep -q LISTEN; then
		echoContent red "\n ---> 443 The port is occupied, please close it manually and then install\n"
		lsof -i tcp:80 | grep LISTEN
		exit 0
	fi
}
...
checkUFWAllowPort() {
	if ufw status | grep -q "$1"; then
		...
		echoContent red " ---> $1 Port open failed"
		exit 0
	fi
}
...
installTools() {
	...
	echoContent green " [Checking] Install updates 【New machines will be slow，If there is no response for a long time，Please stop manually and execute again】"
	...
	if ! find /usr/bin /usr/sbin | grep -q -w nginx; then
		...
		if [[ ${nginxVersion} -lt 14 ]]; then
			read -r -p "Read the current Nginx Version not supported gRPC，will cause the installation to fail，Whether to uninstall and reinstall Nginx ？[y/n]:" unInstallNginxStatus
			if [[ "${unInstallNginxStatus}" == "y" ]]; then
				${removeType} nginx >/dev/null 2>&1
				echoContent yellow " [Uninstall] nginx Complete"
				echoContent green " [Install] nginx"
	...
	if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
		...
		if [[ ! -d "$HOME/.acme.sh" ]] || [[ -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
			echoContent red " [Failed] install acme.sh"
			...
			echoContent red "  2.acme.sh bug in script，viewed [https://github.com/acmesh-official/acme.sh] issues"
			exit 0
		fi
	fi
}
...
installWarp() {
	...
	if [[ -z $(which warp-cli) ]]; then
		echoContent red " [Failed] Install WARP"
		exit 0
	fi
...
checkIP() {
	...
	if [[ -z ${localIP} ]] || ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q '\.' && ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q ':'; then
		...
		echoContent red " Please check if firewall rules are open 443、80\n"
		read -r -p "Whether to modify firewall rules through scripts to open 443、80 Port？[y/n]:" allPortFirewallStatus
		...
		if echo "${localIP}" | awk -F "[,]" '{print $2}' | grep -q "." || echo "${localIP}" | awk -F "[,]" '{print $2}' | grep -q ":"; then
			echoContent red "\n Multiple ip detected，Please confirm whether to close cloudflare the clouds"
	...
	if [[ -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" && -f "/etc/v2ray-agent/tls/${tlsDomain}.key" && -n $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
		...
		if [[ ! -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" || ! -f "/etc/v2ray-agent/tls/${tlsDomain}.key" ]] || [[ -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.key") || -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt") ]]; then
			...
			if [[ ${installTLSCount} == "1" ]]; then
				echoContent red " [Failed] TLS installation，Please check the acme log"
				exit 0
			fi
			echoContent red " [Failed] TLS installation，Checking 80、443 Is the port open"
			allowPort
			echoContent yellow " [Retry] installing the TLS certificate"
			installTLSCount=1
			installTLS "$1"
		fi
		echoContent green " [Success] TLS generate"
	else
...
initNginxConfig() {
	echoContent skyBlue "\n Progress  $1/${totalProgress} : Deploy Nginx"
...
randomPathFunction() {
	...
	if [[ -n "${currentPath}" ]]; then
		echo
		read -r -p "Read to last installation record，Whether to use the path from the last installation ？[y/n]:" historyPathStatus
		echo
	fi
...
nginxBlog() {
	...
	if [[ -d "/usr/share/nginx/html" && -f "/usr/share/nginx/html/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			echoContent green " [Succes] Add fake site"
		fi
	else
		randomNum=$((RANDOM % 6 + 1))
		rm -rf /usr/share/nginx/html
...
handleNginx() {
	...
	if [[ -z $(pgrep -f "nginx") ]] && [[ "$1" == "start" ]]; then
		...
		if [[ -z $(pgrep -f nginx) ]]; then
			echoContent red " [Failed] Start Nginx"
...
installCronTLS() {
	...
	echoContent green "\n [Succes] Add the scheduled maintenance certificate"
}
...
renewalTLS() {
	...
	if [[ -d "$HOME/.acme.sh/${domain}_ecc" ]] && [[ -f "$HOME/.acme.sh/${domain}_ecc/${domain}.key" ]] && [[ -f "$HOME/.acme.sh/${domain}_ecc/${domain}.cer" ]]; then
		...
		echoContent skyBlue " [Checking] Certificate Date:$(date "+%F %H:%M:%S")"
		echoContent skyBlue " [Generate] Certificate Date:$(date -d @"${modifyTime}" +"%F %H:%M:%S")"
		echoContent skyBlue " [Generate] Certificate Days:${days}"
		echoContent skyBlue " [Remaining] Certificate Days:"${tlsStatus}
		...
		echoContent red " [Failed] Not Install"
	fi
}
...
installV2Ray() {
	...
	if [[ "${coreInstallType}" != "2" && "${coreInstallType}" != "3" ]]; then
		...
		echoContent green " [Version] v2ray-core:${version}"
		...
		if [[ "${selectCoreType}" == "3" ]]; then
			echoContent green " [Locking] v2ray-core version v4.32.1"
			rm -f /etc/v2ray-agent/v2ray/v2ray
			rm -f /etc/v2ray-agent/v2ray/v2ctl
			installV2Ray "$1"
		else
			echoContent green " [version] v2ray-core:$(/etc/v2ray-agent/v2ray/v2ray --version | awk '{print $2}' | head -1)"
...
installXray() {
	...
	if [[ "${coreInstallType}" != "1" ]]; then
		...
		echoContent green " [Version] Xray-core:${version}"
		if wget --help | grep -q show-progress; then
		...
		echoContent green " [Version] Xray-core:$(/etc/v2ray-agent/xray/xray --version | awk '{print $2}' | head -1)"
...
v2rayVersionManageMenu() {
	...
	if [[ "${selectV2RayType}" == "1" ]]; then
		...
		echoContent yellow "3.If the rolled back version does not support the current config，Will not be able to connect，Proceed with caution"
...
xrayVersionManageMenu() {
	...
	echoContent yellow "2.Go back Xray-core"
	...
	if [[ -z "${coreInstallType}" ]]; then
		...
		if [[ -n "$1" ]]; then
			...
			if [[ "${installV2RayStatus}" == "y" ]]; then
				...
				echoContent green " [Success] Update"
			fi
...
checkGFWStatue() {
	...
	if [[ "${coreInstallType}" == "1" ]] && [[ -n $(pgrep -f xray/xray) ]]; then
		...
		echoContent red " [Failed] Service start, please check whether there is a log print on the terminal"
		exit 0
	fi
...
handleV2Ray() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "v2ray/v2ray") ]]; then
			echoContent green " [Started] V2Ray successfully"
		else
			echoContent red "[Failed] Start V2Ray"
			echoContent red "Please check manually【/etc/v2ray-agent/v2ray/v2ray -confdir /etc/v2ray-agent/v2ray/conf】，View the error log"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
		if [[ -z $(pgrep -f "v2ray/v2ray") ]]; then
			echoContent green " [Closed] V2Ray successfully"
		else
			echoContent red "[Failed] Close V2Ray"
			echoContent red "Please check manually【ps -ef|grep -v grep|grep v2ray|awk '{print \$2}'|xargs kill -9】"
			exit 0
		fi
	fi
}
...
handleXray() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "xray/xray") ]]; then
			echoContent green " [Started] Xray successfully"
		else
			echoContent red "[Failed] Start Xray"
			echoContent red "Please check manually【/etc/v2ray-agent/xray/xray -confdir /etc/v2ray-agent/xray/conf】，View the error log"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
		if [[ -z $(pgrep -f "xray/xray") ]]; then
			echoContent green " [Closed] Xray successfully"
		else
			echoContent red "[Failed] Close Xray"
			echoContent red "Please check manually【ps -ef|grep -v grep|grep xray|awk '{print \$2}'|xargs kill -9】"
			exit 0
		fi
	fi
}

# initialization V2Ray configuration file
initV2RayConfig() {
...
# Initialization Xray Trojan XTLS configuration file
initXrayFrontingConfig() {
	...
	echoContent yellow "Execute again to switch to the previous problem\n"
...
customCDNIP() {
	...
	echoContent yellow " 2.Move:www.cloudflare.com"
	echoContent yellow " 3.Move:www.digitalocean.com"
	echoContent skyBlue "----------------------------"
...
defaultBase64Code() {
	...
	if [[ "${type}" == "vlesstcp" ]]; then
		...
		if [[ "${coreInstallType}" == "1" ]] && echo "${currentInstallProtocolType}" | grep -q 0; then
			echoContent yellow " ---> Format Account (VLESS+TCP+TLS/xtls-rprx-direct)"
...
EOF
			...
			echoContent yellow " ---> Format Account (VLESS+TCP+TLS/xtls-rprx-splice)"
...
EOF
			...
			echoContent yellow " ---> Format Account (VLESS+TCP+TLS)"
...
EOF
		...
		echoContent yellow " ---> Format Account (Trojan+TCP+TLS/xtls-rprx-direct)"
...
EOF
		...
		echoContent yellow " ---> Format Account (Trojan+TCP+TLS/xtls-rprx-splice)"
...
EOF
		...
		echoContent yellow " ---> Format Account (VMess+WS+TLS)"
...
EOF
		...
		echoContent yellow " ---> Format Account (VMess+TCP+TLS)"
		echoContent green "    vmess://${qrCodeBase64Default}\n"
...
EOF
		...
		echoContent yellow " ---> Format Account (VLESS+WS+TLS)"
...
EOF
		...
		echoContent yellow " ---> Format Account (VLESS+gRPC+TLS)"
	...
	if [[ -n "${configPath}" ]]; then
		...
		if echo ${currentInstallProtocolType} | grep -q 1; then
			...
			jq .inbounds[0].settings.clients ${configPath}03_VLESS_WS_inbounds.json | jq -c '.[]' | while read -r user; do
				...
				#				if [[ ${coreInstallType} == "1" ]]; then
				#					echoContent yellow "Xray 0-RTT path There will be later, it is not compatible with the v2ray-centric client, please delete it manually and use it\n"
				#					path="${currentPath}ws"
				#				fi
...
# Update disguise station
updateNginxBlog() {
	echoContent skyBlue "\n Progress $1/${totalProgress} : Change the camouflage site"
	echoContent red "=============================================================="
	...
	if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
		...
		echoContent green " [Success] The replacement of the fake station"
	else
...
# Add new port
addCorePort() {
	echoContent skyBlue "\n Progress 1/${totalProgress} : Add new port"
	echoContent red "\n=============================================================="
	echoContent yellow "# Precautions\n"
	...
	echoContent yellow "Example:2053,2083,2087\n"
...
unInstall() {
	...
	handleNginx stop
	if [[ -z $(pgrep -f "nginx") ]]; then
		echoContent green " [Success] Stop Nginx"
	fi
	...
	rm -rf /root/.acme.sh
	echoContent green " [Success] Delete acme.sh"
	rm -rf /etc/systemd/system/v2ray.service
	echoContent green " [Success] Deletion of V2Ray"
	...
	if [[ -d "/etc/v2ray-agent/tls" ]] && [[ -n $(find /etc/v2ray-agent/tls/ -name "*.key") ]] && [[ -n $(find /etc/v2ray-agent/tls/ -name "*.crt") ]]; then
		...
		if [[ -n $(find /tmp/v2ray-agent-tls -name '*.key') ]]; then
			echoContent yellow " [Success] Backup certificate, please keep it [/tmp/v2ray-agent-tls]"
		fi
	fi
	...
	echoContent green " [Success] Uninstall shortcut"
	echoContent green " [Success] Uninstall v2ray-agent script"
}
	...
	if [[ -n "${currentAdd}" ]]; then
		...
		if [[ -n ${setDomain} ]]; then
			...
			if [[ $(jq -r .inbounds[0].settings.clients[0].add ${configPath}${frontingType}.json) == "${setDomain}" ]]; then
				echoContent green " [Success] CDN modified"
				reloadCore
			else
				echoContent red " [Failed] Modify CDN"
			fi
		fi
	else
		echoContent red " Available types not installed"
...
handleFirewall() {
	if systemctl status ufw 2>/dev/null | grep -q "active (exited)" && [[ "$1" == "stop" ]]; then
		...
		echoContent green " [Success] Ufw closed"
	...
	if systemctl status firewalld 2>/dev/null | grep -q "active (running)" && [[ "$1" == "stop" ]]; then
		...
		echoContent green " [Success] Firewalld shut down"
	fi
}
...
bbrInstall() {
	echoContent red "\n=============================================================="
	echoContent green "BBR、DD for script[ylx2016]mature works，address[https://github.com/ylx2016/Linux-NetSpeed]，please select"
	echoContent yellow "1.Install script【Recommended original BBR+FQ】"
	echoContent yellow "2.Back home directory"
	echoContent red "=============================================================="
...
aliasInstall() {
	...
	if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "作者：mack-a"; then
		...
		if [[ "${vasmaType}" == "true" ]]; then
			echoContent green "The shortcut was created successfully, you can execute [vasma] to open the script"
		fi
	fi
}
...
checkIPv6() {
	...
	if [[ -z "${pingIPv6}" ]]; then
		echoContent red " [Info] Not Support ipv6"
		exit 0
	fi
}
...
ipv6Routing() {
	if [[ -z "${configPath}" ]]; then
		echoContent red " [Info] Not installed, please use script to install"
		menu
		exit 0
	fi
	...
	if [[ "${ipv6Status}" == "1" ]]; then
		...
		echoContent yellow "3.If the kernel fails to start, please check the domain and add the domain again"
		echoContent yellow "4.Special characters are not allowed, pay attention to the format of command"
		echoContent yellow "5.Every time you add it, it will be added again, and the last domain will not be retained"
		echoContent yellow "6.Example:google,youtube,facebook\n"
		read -r -p "Please enter the domain according to the example above:" domainList
...
EOF
		...
		echoContent green " [Info] Added successfully"
		...
		echoContent green " [Info] IPv6 offloading succeeded"
	else
		echoContent red " Wrong selection"
		exit 0
	fi
...
btTools() {
	...
	if [[ -f ${configPath}09_routing.json ]] && grep -q bittorrent <${configPath}09_routing.json; then
		...
		echoContent yellow "Current state: Enabled"
	fi
...
EOF
		...
		echoContent green " [Info] BT download disabled successfully"
		...
		echoContent green " [Info] BT download opened successfully"
	else
		echoContent red " Wrong selection"
		exit 0
	fi
...
blacklist() {
	...
	if [[ "${blacklistStatus}" == "1" ]]; then
		...
		echoContent yellow "6.Example:speedtest,facebook\n"
...
EOF
		...
		echoContent green " [Info] Domain blacklist deleted successfully"
	else
		echoContent red " Wrong selection"
		exit 0
	fi
...
warpRouting() {
	...
	echoContent red "=============================================================="
	#	echoContent yellow "# Precautions\n"
	#	echoContent yellow "1.The official warp has bugs after several rounds of testing. Restarting the warp will cause the warp to fail and fail to start. It is also possible that the CPU usage will skyrocket."
	...
	#	echoContent yellow "4.Can not be used after restart, you can also uninstall and reinstall"
	# install warp
	if [[ -z $(which warp-cli) ]]; then
		echo
		...
		if [[ "${installCloudflareWarpStatus}" == "y" ]]; then
			...
			echoContent yellow " [Info] Abandon the installation"
			exit 0
		fi
	fi
	...
	if [[ "${warpStatus}" == "1" ]]; then
		...
		echoContent yellow "5.Special characters are not allowed, pay attention to the format of command"
...
EOF
		...
		echoContent green " [Info] WARP offload successfully"
	else
		echoContent red " Wrong selection"
		exit 0
	fi
...
dokodemoDoorUnblockStreamingMedia() {
	echoContent skyBlue "\n Progress 1/${totalProgress} : Any Door Unlock Streaming"
	echoContent red "\n=============================================================="
	echoContent yellow "# Precautions"
...
setDokodemoDoorUnblockStreamingMediaOutbounds() {
	...
	if [[ -z ${domainList} ]]; then
		echoContent red " [Info] Domain name cannot be empty"
		setDokodemoDoorUnblockStreamingMediaOutbounds
	fi
...
EOF
		...
		reloadCore
		echoContent green " [Info] Add outbound unlock successfully"
		exit 0
	fi
	echoContent red " [Info] ip cannot be empty"
}
...
setDokodemoDoorUnblockStreamingMediaInbounds() {
	...
	echoContent yellow "3.If the kernel fails to start, please check the domain name and add the domain name again"
	echoContent yellow "4.Special characters are not allowed, pay attention to the format of command"
	echoContent yellow "5.Re-add every time，The last domain name will not be retained"
	echoContent yellow "6.Ip example:1.1.1.1,1.1.1.2"
	echoContent yellow "7.The following domain name must be consistent with the outbound vps"
	echoContent yellow "8.Example:netflix,disney,hulu\n"
...
EOF
		...
		reloadCore
		echoContent green " [Info] Add landing machine inbound unlock successfully"
		exit 0
	fi
	echoContent red " [Info] ip cannot be empty"
}
...
removeDokodemoDoorUnblockStreamingMedia() {
	...
	reloadCore
	echoContent green " [Info] Uninstalled successfully"
}
...
EOF
		...
		echoContent yellow "\n [Info] If you still can't watch it, you can try the following two solutions"
		...
		echoContent red " [Info] Dns cannot be null"
	fi
	exit 0
}
...
customV2RayInstall() {
	...
	if [[ "${selectCustomInstallType}" =~ ^[0-5]+$ ]]; then
		...
		echoContent red " [Info] Invalid input"
		customV2RayInstall
	fi
}
...
customXrayInstall() {
	...
	echoContent yellow "4.Trojan"
	echoContent yellow "5.VLESS+TLS+gRPC[CDN]"
	read -r -p "Please select [Multiple choice]，[例如:123]:" selectCustomInstallType
	echoContent skyBlue "--------------------------------------------------------------"
	if [[ -z ${selectCustomInstallType} ]]; then
		echoContent red " [Info] Not nullable"
		customXrayInstall
...
selectCoreInstall() {
	...
	3)
		...
		;;
	*)
		echoContent red ' [Info] Choose wrong, choose again'
		selectCoreInstall
		;;
	esac
}
...
coreVersionManageMenu() {
	...
	if [[ -z "${coreInstallType}" ]]; then
		echoContent red "\n [Info] No installation directory detected, please execute the script to install the content"
		menu
		exit 0
	fi
	if [[ "${coreInstallType}" == "1" ]]; then
...
subscribe() {
	if [[ -n "${configPath}" ]]; then
		...
		echoContent red " [Info] Not Installed"
	fi
}
...
switchAlpn() {
	...
	if [[ -z ${currentAlpn} ]]; then
		echoContent red " [Info] Unable to read alpn, please check if it is installed"
		exit 0
	fi

	echoContent red "\n=============================================================="
	echoContent green "The current alpn first is：${currentAlpn}"
	echoContent yellow "  1.When http/1.1 is the first, trojan is available, and some RPC clients are available [client supports manual selection of alpn available]"
...
menu() {
	...
	echoContent green "Original：Mack-a"
	echoContent green "Version：v2.5.51"
	...
	echoContent yellow "12.BT management"
