...
EOF
			...
			echoContent green "    agreement type：VLESS，address：${host}，port：${port}，User ID：${id}，Safety ：xtls，transfer method：tcp，flow：xtls-rprx-splice，account name:${email/direct/splice}\n"
			cat <<EOF >>"/etc/v2ray-agent/subscribe_tmp/${subAccount}"
