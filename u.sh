...
EOF
		...
		qrCodeBase64Default=$(echo -n "{\"port\":${port},\"ps\":\"${email}\",\"tls\":\"tls\",\"id\":\"${id}\",\"aid\":0,\"v\":2,\"type\":\"grpc\",\"serviceName\":\"${path}\",\"net\":\"grpc\",\"add\":\"${add}\",\"allowInsecure\":0,\"method\":\"none\",\"sni\":\"${host}\"}" | base64 -w 0)
		qrCodeBase64Default="${qrCodeBase64Default// /}"
