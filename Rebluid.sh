...
installTools() {
	...
	echoContent skyBlue "\n进度  $1/${totalProgress} : installation tool"
...
checkIP() {
	...
	if [[ -z ${localIP} ]] || ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q '\.' && ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q ':'; then
		echoContent red "\n xxxx> The ip of the current domain name was not detected"
...
EOF
		...
		echoContent green "    trojan://${id}@${host}:${port}?encryption=none&peer=${host}&security=tls&type=grpc&sni=${host}&alpn=h2&path=${path}&serviceName=${path}#${host}\n"
...
customV2RayInstall() {
	...
	echoContent yellow "6.VMess+TLS+gRPC[CDN]"
	...
	if [[ "${selectCustomInstallType}" =~ ^[0-6]+$ ]]; then
		cleanUp xrayClean
		totalProgress=17
		installTools 1
		# apply for tls
...
customXrayInstall() {
	...
	if [[ -z ${selectCustomInstallType} ]]; then
		...
		handleNginx stop
		# random path
		if echo "${selectCustomInstallType}" | grep -q 1 || echo "${selectCustomInstallType}" | grep -q 2 || echo "${selectCustomInstallType}" | grep -q 3 || echo "${selectCustomInstallType}" | grep -q 6 ||  echo "${selectCustomInstallType}" | grep -q 5; then
			randomPathFunction 5
			customCDNIP 6
		fi
		nginxBlog 7
