...
EOF
	...
	if echo "${selectCustomInstallType}" | grep -q 6 || [[ "$1" == "all" ]]; then
		if ! echo "${selectCustomInstallType}" | grep -q 6 && [[ -n ${selectCustomInstallType} ]]; then
...
EOF
		...
		qrCodeBase64Default=$(echo -n "{\"port\":${port},\"ps\":\"${email}\",\"tls\":\"tls\",\"id\":\"${id}\",\"aid\":0,\"v\":2,\"host\":\"${host}\",\"type\":\"grpc\",\"serviceName\":\"/${path}\",\"net\":\"grpc\",\"add\":\"${add}\",\"allowInsecure\":0,\"method\":\"gun\",\"sni\":\"${host}\"}" | base64 -w 0)
		qrCodeBase64Default="${qrCodeBase64Default// /}"
...
showAccounts() {
	...
	if [[ -n "${configPath}" ]]; then
		...
		if echo ${currentInstallProtocolType} | grep -q 6; then
			echoContent skyBlue "\n———————————————————————————————— VMess gRPC ———————————————————————————————— \n"
			local serviceName
			serviceName=$(jq -r .inbounds[0].streamSettings.grpcSettings.serviceName ${configPath}07_VMess_gRPC_inbounds.json)
			jq .inbounds[0].settings.clients ${configPath}07_VMess_gRPC_inbounds.json | jq -c '.[]' | while read -r user; do
				echoContent skyBlue "\n ---> account number：$(echo "${user}" | jq -r .email)_$(echo "${user}" | jq -r .id)"
				echo
				defaultBase64Code vmessgrpc "$(echo "${user}" | jq -r .email)" "$(echo "${user}" | jq -r .id)" "${currentHost}:${currentPort}" "${serviceName}" "${currentAdd}"
			done
		fi
	fi
